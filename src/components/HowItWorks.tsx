...
const points = [
  { icon: ShieldCheck, title: 'Start with the agreed trade', desc: 'Make sure both traders have confirmed exactly what is being exchanged before anything is sent.' },
  { icon: Users, title: 'Open an FSMM request', desc: 'For an eligible trade, request a middleman through the official FSMM server and follow the ticket instructions.' },
  { icon: Scale, title: 'Keep the exchange neutral', desc: 'The middleman follows the confirmed deal and helps keep the exchange organized for both sides.' },
];
    ...
    <section id="how-it-works" className="relative py-20 sm:py-28 bg-gradient-to-b from-brand-50/30 to-white/40 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <div className="text-center max-w-3xl mx-auto reveal">
          ...
          <h2 className="mt-3 font-display text-3xl sm:text-4xl font-extrabold text-brand-950 tracking-tight">A clear process for eligible trades</h2>
          <p className="mt-4 text-lg text-slate-600">When a trade calls for a middleman, FSMM provides a straightforward process with an independent person overseeing the exchange.</p>
        </div>
        <div className="mt-14 grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {points.map((item, i) => (
            <div key={item.title} className={`group rounded-2xl bg-white/90 backdrop-blur border border-brand-100 p-7 shadow-sm hover:shadow-xl hover:shadow-brand-600/10 hover:-translate-y-1.5 hover:border-brand-200 transition-all duration-500 reveal reveal-delay-${i + 1}`}>
              ...
              <p className="mt-5 text-xs font-bold uppercase tracking-wider text-brand-600">Step {i + 1}</p>
