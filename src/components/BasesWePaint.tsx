...
const bases = [
  { name: 'Candy', requirement: '100% of the eligible Candy Index', status: 'Past mutation index', image: '/bases/candy.svg' },
  { name: 'Lava', requirement: '100% of the eligible Lava Index', status: 'Past mutation index', image: '/bases/lava.svg' },
  { name: 'Galaxy', requirement: '100% of the eligible Galaxy Index', status: 'Past mutation index', image: '/bases/galaxy.svg' },
  { name: 'Yin Yang', requirement: '75% of the eligible Yin Yang Index', status: 'Past mutation index', image: '/bases/yin-yang.svg' },
  { name: 'Radioactive', requirement: '75% of the eligible Radioactive Index', status: 'Past mutation index', image: '/bases/radioactive.svg' },
  { name: 'Cursed', requirement: '60% of the eligible Cursed Index', status: 'Past mutation index', image: '/bases/cursed.svg' },
  { name: 'Divine', requirement: '60% of the eligible Divine Index', status: 'Past mutation index', image: '/bases/divine.svg' },
  { name: 'Cyber', requirement: '60% of the eligible Cyber Index', status: 'Past mutation index', image: '/bases/cyber.svg' },
  { name: 'Phantom', requirement: '60% of the eligible Phantom Index', status: 'Check the live Index', image: '/bases/phantom.svg' },
  { name: 'Crystal', requirement: '50% of the Crystal Index', status: 'Newer mutation base', image: '/bases/crystal.svg' },
];
          ...
          {bases.map((base, i) => (
            <button key={base.name} type="button" onClick={() => setSelected(base)} className={`group relative overflow-hidden rounded-3xl border border-brand-100 bg-white/95 p-4 text-left shadow-sm hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand-600/15 transition-all duration-500 reveal reveal-delay-${(i % 4) + 1}`}>
              ...
              <div className="aspect-square rounded-2xl bg-slate-50 border border-slate-100 overflow-hidden flex items-center justify-center">
                <img src={base.image} alt={`${base.name} base skin`} className="h-full w-full object-contain group-hover:scale-105 transition-transform duration-500" loading="lazy" />
              </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-5" role="dialog" aria-modal="true" aria-label={`${selected.name} base details`}>
          ...
          <div className="relative w-full max-w-2xl overflow-hidden rounded-[30px] border border-brand-100 bg-white shadow-2xl reveal is-visible">
            ...
            <div className="grid sm:grid-cols-[.8fr_1.2fr]">
              <div className="bg-slate-50 p-8 flex items-center justify-center min-h-64 sm:min-h-full"><img src={selected.image} alt="" className="max-h-64 w-full object-contain" /></div>
