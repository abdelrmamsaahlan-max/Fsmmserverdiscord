...
// Keep all base artwork local to the Vercel build. No Fandom/CDN requests means
// the cards cannot break because an external media redirect changes or blocks hotlinking.
const bases = [
  { name: 'Candy', requirement: '100% of the eligible Candy Index', status: 'Past mutation index', image: '/bases/candy.svg?v=2' },
  { name: 'Lava', requirement: '100% of the eligible Lava Index', status: 'Past mutation index', image: '/bases/lava.svg?v=2' },
  { name: 'Galaxy', requirement: '100% of the eligible Galaxy Index', status: 'Past mutation index', image: '/bases/galaxy.svg?v=2' },
  { name: 'Yin Yang', requirement: '75% of the eligible Yin Yang Index', status: 'Past mutation index', image: '/bases/yin-yang.svg?v=2' },
  { name: 'Radioactive', requirement: '75% of the eligible Radioactive Index', status: 'Past mutation index', image: '/bases/radioactive.svg?v=2' },
  { name: 'Cursed', requirement: '60% of the eligible Cursed Index', status: 'Past mutation index', image: '/bases/cursed.svg?v=2' },
  { name: 'Divine', requirement: '60% of the eligible Divine Index', status: 'Past mutation index', image: '/bases/divine.svg?v=2' },
  { name: 'Cyber', requirement: '60% of the eligible Cyber Index', status: 'Past mutation index', image: '/bases/cyber.svg?v=2' },
  { name: 'Phantom', requirement: '60% of the eligible Phantom Index', status: 'Check the live Index', image: '/bases/phantom.svg?v=2' },
];

type Base = typeof bases[number];

function BaseImage({ base, className, alt }: { base: Base; className: string; alt: string }) {
  ...
  return <img src={base.image} alt={alt} className={className} loading="eager" decoding="async" onError={() => setFailed(true)} />;
}
          ...
          {bases.map((base, i) => (
            <button key={base.name} type="button" onClick={() => setSelected(base)} className={`group relative overflow-hidden rounded-3xl border border-brand-100 bg-white/95 p-4 text-left shadow-sm hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand-600/15 transition-all duration-500 reveal reveal-delay-${(i % 4) + 1}`}>
              ...
              <div className="aspect-square rounded-2xl bg-slate-50 border border-slate-100 overflow-hidden flex items-center justify-center">
                <BaseImage base={base} alt={`${base.name} base skin`} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-500" />
              </div>
